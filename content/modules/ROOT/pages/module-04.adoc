...
In the link:tab-1[button label=``rhelvm''] terminal, run `nc`, listening
on port 9999.
...
`the quick brown fox jumps over the lazy dogs` followed by the `enter`
key.

In the link:tab-0[button label=``controlnode''] terminal, type `ctrl-c`
to exit `nc`.
